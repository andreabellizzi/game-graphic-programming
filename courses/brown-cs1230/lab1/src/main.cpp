...
struct Circle {
    ...
    double calculateArea() {
        return std::numbers::pi * radious * radious;
    }

    double calculatePerimeter() {
        return 2 * radious * std::numbers::pi;
    }
};
...
void printShape(auto shape) {
    if constexpr (requires {shape.radious;}) {
        std::cout << "Circle area " << shape.calculateArea() << std::endl;
    } else {
        std::cout << "Rectangle area " << shape.calculateArea() << std::endl;
    }
}

// Task 9:
void inplaceTimesTwo(auto* a) {
    *a = *a + *a;
}

// Task 10:
void doubleEachElement(auto& a)  {
    for(auto& x : a) {
        x = x + x;
    }
}
...
int main() {
    ...
    std::cout << "Hello World from my first Qt project!" << std::endl;
    ...
    Circle c1 = { .radious = 3.2 };
    Circle c2 = { .radious = 2 };
    std::cout << "c1 area " << c1.calculateArea() << " c1 perimeter " << c1.calculatePerimeter() << std::endl;
    std::cout << "c2 area " << c2.calculateArea() << " c2 perimeter " << c2.calculatePerimeter() << std::endl;

    // Task 7: call printShape with different Rectangle and Circle objects
    printShape(c1);
    printShape(Rectangle{.length=3, .width=4});
    ...
    std::array<std::string, 3> str_array = {"str1", "str2", "str3"};
    for(auto& x : str_array) {
        x = timesTwo(x);
    }
    ...
    std::cout << ptrxx << std::endl; //equal to &xx
    std::cout << &ptrxx << std::endl;
    std::cout << rxx << std::endl;
    std::cout << &rxx << std::endl; //equal to &xx
